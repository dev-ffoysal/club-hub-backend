  ...
  const fileFilter = (
    ...
    try {
      const allowedTypes = {
        ...
        covers: ['image/jpeg', 'image/png', 'image/jpg'],
        ...
        if (req.files) {
          ...
          for (const [fieldName, files] of Object.entries(req.files)) {
            ...
            for (const file of fileArray) {
              ...
              if (fieldName === 'images' || fieldName === 'covers' && file.mimetype.startsWith('image/')) {
                try {
...
export const fileAndBodyProcessorUsingDiskStorage = () => {
  ...
  const storage = multer.diskStorage({
    destination: (req, file, cb) => {
      ...
      console.log("FROM FILE PROCESS: 😀😀😀😀😀😀😀😀", req.files, req.file, req.body)
